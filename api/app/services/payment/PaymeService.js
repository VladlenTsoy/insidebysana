const {paymeLogger} = require("config/logger.config")
const {PaymeTransaction} = require("models/payments/PaymeTransaction")
const {Order} = require("models/orders/Order")
const moment = require("moment")
const OrderEvents = require("_events/OrderEvents")
...
class PaymeService {
    ...
    // Проверка транзакции
    async checkTransaction(find) {
        try {
            if (find) return await PaymeTransaction.query().findOne(find)

            const {id} = this.params
            const transaction = await PaymeTransaction.query().findOne({paycom_transaction_id: id})
    ...
    async CreateTransaction() {
        try {
            ...
            if (!transaction) {
                ...
                if (checkTimeout)
                    return this.error(this.PaycomException.ERROR_COULD_NOT_PERFORM, "Transaction is expired.")
